...
export const FEATURES = {
  ...
  b2bShops: true,
  // b2bStorefront — Фазы B2/B3: публичная витрина /s/<slug>, кабинет владельца /shop-admin,
  // создание магазина из заявки (admin approve→shop), buy-flow с атрибуцией. ВКЛЮЧЕНА
  // (миграции 0066/0067 применены в проде 2026-07-02). Деньги магазина (доля→баланс→выплаты)
  // — фаза B4. Аноним читает витрину через RPC shop_storefront (RLS не светит access_code).
  b2bStorefront: true,
} as const
...
export const FEATURE_ROUTES: { prefix: string; enabled: boolean }[] = [
  ...
  { prefix: '/shop-new', enabled: FEATURES.b2bStorefront },
  { prefix: '/shop-claim', enabled: FEATURES.b2bStorefront },
]
