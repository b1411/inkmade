-- 0048: apply_paid v3 — guard перехода состояния (инвариант §10: только pending→paid).
-- Раньше apply_paid выставлял status='paid' независимо от текущего статуса: валидный
-- webhook мог провести в paid даже отменённый заказ. Теперь оплата применяется только
-- к заказам в оплачиваемом статусе ('created'/'pending'); прочие (в т.ч. 'cancelled')
-- отклоняются. Идемпотентность по paid_at сохранена и проверяется раньше (повторный
-- webhook по уже оплаченному заказу возвращает already_paid без ошибки).
-- Тело идентично 0026 + добавлен только guard статуса после проверки paid_at.
