-- 0052: apply_paid v4 — атомарная проверка остатка склада (анти-оверселл).
-- Аудит 2026-06-29, CRITICAL #3. Раньше списание было `stock = greatest(0, stock - qty)`:
-- два заказа за последнюю единицу оба проходили, остаток молча упирался в 0 (оверселл).
-- Advisory-lock в apply_paid берётся по ЗАКАЗУ (hashtext(order_id)), поэтому разные
-- заказы за один вариант НЕ сериализуются между собой — гонку нельзя поймать им.
-- Решение: атомарный guarded-decrement `update ... where stock >= qty` — row-lock
-- варианта сериализует конкурентные обновления; если строк не затронуто (остатка нет) —
-- raise, и вся транзакция оплаты откатывается (заказ остаётся неоплаченным).
--
-- ВАЖНО (для будущего реального провайдера): сейчас платёж — mock, поэтому raise
-- после «оплаты» безопасен. С реальным провайдером деньги уже захвачены к моменту
-- webhook → apply_paid; чтобы не отбивать оплаченный заказ, нужно либо РЕЗЕРВИРОВАТЬ
-- остаток при создании заказа, либо заменить здесь raise на отметку backorder.
-- Тело идентично 0048, изменён ТОЛЬКО блок списания склада.
