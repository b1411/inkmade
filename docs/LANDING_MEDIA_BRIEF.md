...
|---|---|---|
| `public/media/ink.svg` (вектор, 9 path) | ✅ **мастер** | источник ВСЕГО набора иконок; `npm run icons` |
| `public/media/ink.png` (2048², альфа) | ✅ есть | растровый дубль мастера от дизайнера, в сборке не участвует |
| `public/media/wordmark-dark.svg` · `wordmark-light.svg` | ✅ **мастер** | надпись «INKMADE» в двух заливках; `npm run logo` |
| `public/logo-dark.svg` · `logo-light.svg` (1328×305) | ✅ есть | шапка/футер/логин/кабинеты; режется из мастеров |
| `public/favicon.svg` | ✅ есть | nuxt.config.ts — **основная фавиконка**, копия мастера |
| `public/favicon.ico` (16/32/48, альфа) | ✅ есть | nuxt.config.ts — запасная для Safari/старья |
| `public/apple-touch-icon.png` (180²) | ✅ есть | nuxt.config.ts — кремовая плашка **без альфы** |
| `public/icon-192.png` · `icon-512.png` | ✅ есть | site.webmanifest (`purpose: any`, круг с альфой) |
| `public/icon-maskable-512.png` | ✅ есть | site.webmanifest (`purpose: maskable`, круг ужат до 79%) |
| `public/og-default.jpg` (1200×630) | ✅ есть | app.vue (глоб. OG); JSON-LD логотип = `icon-512.png` |
| `public/og/og-business.jpg` | ❌ нет | своя OG для `/business` (сейчас берёт дефолт) |

### 0.1 Логотип-знак / favicon — ✅ **СЛОТ ЗАКРЫТ** (2026-07-15, ред. 2)
**Что принято:** **круглый знак** — бордовый диск `#7A1F28` во весь кадр, кремовые `#EFE0C1` буквы
«INK» кистью, прозрачные углы. Не ярлык-бирка (ред. 1) и не капля (исходный бриф).
Бордо совпадает с `theme_color` пиксель-в-пиксель.

**Мастер — `public/media/ink.svg`, настоящий вектор:** 9 `<path>`, ноль `<image>`, ноль base64,
шрифт переведён в кривые → шрифтовых зависимостей нет, масштабируется без потерь. Рендерится
идентично растровому дублю (расхождение 0.33% субпикселей = только антиалиасинг).
Из него `scripts/gen-icons.mjs` (`npm run icons`) режет ВЕСЬ набор.

**Три разных обращения со знаком — не путать:**

| Слот | Форма | Почему так |
|---|---|---|
| `favicon.svg` (главная) | вектор как есть, круг во весь кадр | подключён **первым** — Chrome/FF/Safari предпочтут SVG растру и отрисуют чётко на любом размере; 16px не мылится |
| `favicon.ico` 16/32/48 | круг с альфой, full-bleed | запасной для Safari и старья; углы прозрачные, ничего не обрезано |
| `apple-touch-icon` 180 | **плашка без альфы**: круг 88% на кремовом | iOS игнорирует прозрачность и красит её чёрным. Скругляет iOS сам — своего скругления не добавляем |
| `icon-192/512` (`any`) | круг с альфой | витрина установки кладёт на свою подложку |
| `icon-maskable-512` | круг **ужат до 92%** на кремовом | буквы уходят на 83% радиуса — маска Android (⌀80%) срезала бы «INK» по бокам |

**Воздух вокруг букв — правка сборки, а не мастера (2026-07-15).** В экспорте буквы упираются в
обод: зазор 4% справа, на 16px это 0.63px — «K» сливается с ободом, и знак читается обрезанным,
хотя не обрезан. Плюс буквы смещены на 21 ед. вправо от центра. `gen-icons.mjs` даёт им воздух и
центрует трансформом вокруг центра холста (`LETTER_SCALE = 0.88`), не трогая круг, обод и цвета:

| | до | после |
|---|---|---|
| буквы от ширины | 89% | **79%** |
| зазор до обода | 4% (0.63px на 16px) | **10.7%** (1.7px) |
| дальний пиксель | 95% радиуса | **83%** |
| круг на Android | 79% холста | **92%** |

- **`ink.svg` не правим руками — это экспорт дизайнера.** Вся правка живёт в `LETTER_SCALE`:
  0.92 — теснее, 0.84 — просторнее. `favicon.svg` собирается уже с воздухом, а не копируется сырым.
- Скрипт держится за строку `LETTERS_GROUP` в экспорте. Переэкспортируете знак из Canva —
  структура может поехать, скрипт **упадёт с внятной ошибкой**, а не молча соберёт кривое.
- **Ужатие под maskable скрипт считает сам:** ищет самый дальний кремовый (=буква) пиксель от
  центра и масштабирует круг так, чтобы он сел в ⌀80% с запасом 4%. Дали буквам воздух — круг на
  Android автоматически вырос с 79% до 92%.
- Зазор печатается в лог каждой сборки — если знак переэкспортируют и буквы снова прилипнут к
  ободу, это видно сразу, а не через полгода в табе.
- **Кремовая плашка обязательна там, где нужна непрозрачность:** знак сам бордовый, на бордовом фоне
  круг растворился бы. Кремовый — второй цвет бренда, даёт Canva-подобную плитку.
- **Осталось (опц.):** `logo-mark.svg` для упаковки/печати — можно просто скопировать мастер.

#### Спека на новый мастер знака (если знак перерисовывается)

| Параметр | Значение | Почему |
|---|---|---|
| Формат | **SVG** (вектор), PNG 2048² вторым | вектор = чёткая фавиконка на любом размере, без мыла на 16px |
| Шрифт | переведён в кривые | иначе знак развалится там, где шрифта нет |
| Холст | 1:1 | любой другой формат придётся кропить, кроп съест края |
| Safe zone | всё важное — в круге **⌀80%** | требование Android maskable; отступ 10% от края |
| Зона букв | круг **⌀66%** | текст не липнет к краю |
| Палитра | `#7A1F28` + `#EFE0C1` | бордо = `theme_color` |
| Куда | `public/media/` | дальше `npm run icons` |

Про экспорт SVG из Canva: **он не спрашивает размер — это норма, а не ошибка.** У вектора нет
пикселей, размер задаётся при отрисовке. Скачивать SVG «побольше» не нужно.

Альфу в мастере оставлять **можно** (мы её ждём для фавиконки) — непрозрачные плашки под iOS/Android
скрипт соберёт сам. Ломает иконку: тонкие линии, градиенты и тени (грязь на 16px), буквы впритык к
краю круга, слоган или домен внутри знака.

<details><summary>Исходный бриф генерации (архив, слот уже закрыт)</summary>
...
### 0.1a Логотип-надпись (wordmark) — ✅ **СЛОТ ЗАКРЫТ** (2026-07-15)
**Что принято:** граффити-надпись «INKMADE» кистью, буквы в контрастной обводке.
До этого надпись была НЕ ассетом, а текстом, набранным гугл-шрифтом Permanent Marker
(`.ink-logo`) — то есть логотипом бренда работал случайный шрифт с CDN. Теперь это вектор,
а шрифт **удалён из сборки** (`nuxt.config.ts` → `fonts.families`): он грузился только ради лого.

**Мастера — `public/media/wordmark-{dark,light}.svg`.** Это одна и та же надпись в двух заливках.
Имя = цвет САМИХ БУКВ, не фона под ними:

| Мастер | Буквы | Обводка | Куда кладём |
|---|---|---|---|
| `wordmark-dark.svg` → `logo-dark.svg` | `#111111` ink-black | кремовая | на **светлый** фон |
| `wordmark-light.svg` → `logo-light.svg` | `#EFE0C1` ink-cream | чёрная | на **тёмный** фон |

Цвета совпадают с токенами `--color-ink-black` / `--color-ink-cream` пиксель-в-пиксель.
Из мастеров `scripts/gen-logo.mjs` (`npm run logo`) режет оба ассета.

**Что делает сборка — и почему мастер не годится в `<img>` как есть:**

| | в мастере | после `npm run logo` |
|---|---|---|
| холст | 1440×810, надпись 1326×303 в центре | **1328×305** — кадр по буквам |
| воздух | 62% кадра пустота | нет (отступы задаёт вёрстка) |
| вес | 153кб (кривые с 6 знаками) | **33.8кб** (svgo, 1 знак) |

- **Мастера не правим руками — это экспорт дизайнера.** Переэкспортировали надпись — `npm run logo`.
- **Границы букв скрипт меряет РЕНДЕРОМ, а не парсингом `path`:** контрольные точки безье лежат
  снаружи кривой и дали бы кадр с запасом.
- **Точность 1 знак безопасна:** 0.1 единицы при ширине кадра 1326 — сотые доли пикселя на любом
  кегле. Сверено рендером против мастера: расхождение 0.37/255 = только антиалиасинг.
- **Кадр у обоих вариантов ОДИН и отцентрован по буквам — это обязательно.** Шапка держит оба
  варианта в DOM и кросс-фейдит их (подложка едет с тёмного hero на светлое стекло при скролле);
  кадры, разъехавшиеся хотя бы на единицу, дали бы рывок надписи. Сверено: 139×32 @ 16,16 у обоих.

**Где какой вариант (тон = цвет букв):**

| Место | Фон | Вариант | Кегль |
|---|---|---|---|
| Шапка над hero (лендинг, до скролла) | прозрачная поверх тёмного | `light` | h-8 (139×32) |
| Шапка на стекле (скролл / внутренние) | `--color-ink-glass`, светлое | `dark` | h-7 (122×28) |
| Мобильное меню | бордо | `light` | h-8 |
| Футер · страница ошибки | ink-black | `light` | h-7 · h-9 |
| Логин: бордовая панель · моб. над формой | бордо · светлый | `light` · `dark` | h-10 · h-8 |
| Кабинеты admin / studio | тёмная панель | `light` | h-7 |
| Кабинеты account / designer / shop-admin | светлая шапка | `dark` (по умолч.) | h-7 |

Точка подключения одна — `UiAppLogo` (проп `tone`), кроме шапки: там свои два `<img>` ради
кросс-фейда. `width`/`height` на `<img>` = кадр ассета: держит соотношение и убирает CLS.
...
Для демонстрации конструктора использовать только реальную запись интерфейса или статичный фирменный принт. Не использовать постановочные офисы, дизайнеров за планшетом и AI-имитацию рабочего процесса.
...
|---|---|---|---|---|---|
| ~~Логотип-знак/favicon~~ ✅ | `media/ink.svg` → `favicon.*`,`icon-*` | вектор | 1:1 | ✅ сдан | — |
...
```
public/
├── favicon.svg            ✅   ГЛАВНАЯ фавиконка (вектор, копия мастера)
├── favicon.ico            ✅   16/32/48 с альфой — запасная для Safari/старья
├── apple-touch-icon.png   ✅   кремовая плашка БЕЗ альфы (iOS красит альфу чёрным)
├── icon-192/512.png       ✅   круг с альфой · icon-maskable-512.png ✅ круг ужат до 79%
├── og-default.jpg         ✅
├── og/                    ❌   og-business.jpg
├── logo-mark.svg          ❌   (опц., = копия media/ink.svg)
└── media/
    ├── ink.svg       ✅   МАСТЕР знака (вектор) — источник всего набора
    ├── ink.png       ✅   растровый дубль 2048² от дизайнера (в сборке не участвует)
...
1. **🟡 Бренд-мета** — ~~логотип-знак → favicon-набор, apple-touch, PWA-иконки~~ ✅ сдано 2026-07-15
   (§0.1); осталось: OG default (§0.2), OG business (§0.3) — Nano Banana Pro. Знак-ярлык теперь
   style-anchor для бренд-мета: OG-кадры делай в его палитре (бордо `#7A1F28` + кремовый `#EFE0C1`).
