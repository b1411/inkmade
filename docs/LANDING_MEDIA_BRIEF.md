...
|---|---|---|
| `public/media/icon.png` (2000², источник) | ✅ есть | мастер-файл знака, из него режутся все производные |
| `public/favicon.ico` (16/32/48) | ✅ есть | nuxt.config.ts |
| `public/favicon-32.png` · `favicon-192.png` | ✅ есть | nuxt.config.ts (плотный кроп — см. §0.1) |
| `public/apple-touch-icon.png` (180²) | ✅ есть | nuxt.config.ts |
| `public/icon-192.png` · `icon-512.png` | ✅ есть | site.webmanifest (`purpose: any`) |
| `public/icon-maskable-512.png` | ✅ есть | site.webmanifest (`purpose: maskable`) |
| `public/og-default.jpg` (1200×630) | ✅ есть | app.vue (глоб. OG); JSON-LD логотип теперь `icon-512.png` |
| `public/favicon.svg` | ⚠️ устарел | старый знак-капля, отвязан от head/manifest — можно удалить |
| `public/og/og-business.jpg` | ❌ нет | своя OG для `/business` (сейчас берёт дефолт) |
| `public/logo-mark.svg` | ❌ нет (опц.) | вектор знака для упаковки/печати |

### 0.1 Логотип-знак / favicon — ✅ **СЛОТ ЗАКРЫТ** (2026-07-15)
**Что принято:** не капля, а **чёрный ярлык-бирка со шнурком и надписью «INK»** кистевым шрифтом,
кремовый контур на бордо `#7A1F28` (фон совпадает с `theme_color` пиксель-в-пиксель).
Мотив ярлыка бьётся с мерчем/принтом точнее, чем абстрактная капля, и рифмуется с граффити-лого.
Источник: `public/media/icon.png` 2000×2000, без альфы (фон сплошной — для иконок это плюс).

- **Производные режутся из мастера, не рисуются заново.** Скрипт-рецепт: bbox знака → квадратный
  кроп по центру → lanczos3 → `png({ palette: true })`.
- **Мелкие размеры — плотный кроп (pad 10%).** В мастере поля 17% по бокам: при прямом ресайзе в 32px
  надпись «INK» схлопывалась в пятно. Кроп по bbox возвращает читаемость.
- **apple-touch — умеренный кроп (pad 22%):** iOS сам скругляет углы, нужен воздух.
- **maskable — знак вписан в safe-circle (⌀80%):** диагональ bbox = 410px ≤ 409.6 → под любой маской
  Android ничего не срежется. Знак занимает 62% холста — это норма для maskable.
- **Осталось (опц.):** вектор знака (`logo-mark.svg`) для упаковки/печати и идеальных 16px.
...
|---|---|---|---|---|---|
| ~~Логотип-знак/favicon~~ ✅ | `media/icon.png` → `favicon.*`,`icon-*` | PNG | 1:1 → 2000² | ✅ сдан | — |
...
├── favicon.svg            ⚠️   старый знак-капля, отвязан — можно удалить
├── og-default.jpg         ✅
├── og/                    ❌   og-business.jpg
├── logo-mark.svg          ❌   (опц., вектор знака)
└── media/
    ├── icon.png      ✅   мастер знака 2000² (источник производных)
