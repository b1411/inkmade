...
|---|---|---|
| `public/media/ink.svg` (вектор, 9 path) | ✅ **мастер** | источник ВСЕГО набора; `npm run icons` |
...
| `icon-maskable-512` | круг **ужат до 79%** на кремовом | буквы в мастере занимают 89% ширины и уходят на **97% радиуса** — маска Android (⌀80%) срезала бы «INK» по бокам |

- **Скрипт считает ужатие сам, не по константе:** ищет самый дальний кремовый (=буква) пиксель от
  центра и масштабирует круг так, чтобы он сел в ⌀80% с запасом 4%. Перерисуете знак — пересчитается.
