...
<template>
  <div class="border border-ink-gray-200 rounded-lg p-4 space-y-3">
    ...
    <dl class="space-y-1 text-caption text-ink-gray-600">
      <div class="flex justify-between"><dt>Изделие</dt><dd>{{ formatPrice(breakdown.base) }}</dd></div>
      <div v-if="breakdown.material" class="flex justify-between"><dt>Материал</dt><dd>+{{ formatPrice(breakdown.material) }}</dd></div>
      <div v-if="breakdown.method" class="flex justify-between"><dt>Метод нанесения</dt><dd>+{{ formatPrice(breakdown.method) }}</dd></div>
      <div v-if="breakdown.print" class="flex justify-between"><dt>Печать</dt><dd>+{{ formatPrice(breakdown.print) }}</dd></div>
      <div v-if="breakdown.text" class="flex justify-between"><dt>Текст</dt><dd>+{{ formatPrice(breakdown.text) }}</dd></div>
    </dl>
