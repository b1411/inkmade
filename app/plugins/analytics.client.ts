// Рекламные пиксели (§3.5.1). Инициализируются, ТОЛЬКО если:
//   1) задан ID счётчика (env), И
//   2) пользователь дал согласие на аналитику/рекламу (cookie-баннер, opt-in).
// До согласия — ничего не грузим (юр. требование: обработка после согласия).
// При согласии позже (клик в баннере) — инициализируем по watch без перезагрузки.
export default defineNuxtPlugin(() => {
  ...
  let started = false
  function initPixels() {
    if (started || consent.value !== 'accepted') return
    started = true
  ...
  // Согласие уже дано (вернувшийся пользователь) — грузим сразу; иначе ждём клика.
  initPixels()
  watch(consent, initPixels)
})
