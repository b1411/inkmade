// Лёгкие серверные примитивы валидации (UUID/числа). Для валидации тел запросов
// используются zod-схемы в server/utils/schemas.ts; эти хелперы — быстрые проверки
// router-параметров и числовых границ на границе API: NaN/Infinity, отрицательные
// суммы, некорректные UUID. Авторитетные бизнес-инварианты (цена по БД, подпись
// webhook, RLS) остаются в самих хендлерах — это первый рубеж.
